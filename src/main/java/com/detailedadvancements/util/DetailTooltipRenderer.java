...
public class DetailTooltipRenderer {
    ...
    private static final int COLOR_BORDER_LIGHT = 0xFF5A3A7E;
    private static final int COLOR_BORDER_DARK = 0xFF2D1C44;
    ...
    public static void render(GuiGraphicsExtractor extractor, AdvancementHolder holder,
        ...
        // 패널 높이: 위아래 패딩 + 각 줄 + 프로그레스바 영역
        int progressBarExtra = LINE_HEIGHT + 8;
        ...
        // 배경
        extractor.fill(panelX, panelY, panelX + PANEL_WIDTH, panelY + panelHeight, COLOR_BG);

        // 테두리
        extractor.fill(panelX, panelY, panelX + PANEL_WIDTH, panelY + 1, COLOR_BORDER_LIGHT);
        extractor.fill(panelX, panelY + panelHeight - 1, panelX + PANEL_WIDTH, panelY + panelHeight, COLOR_BORDER_DARK);
        extractor.fill(panelX, panelY, panelX + 1, panelY + panelHeight, COLOR_BORDER_LIGHT);
        extractor.fill(panelX + PANEL_WIDTH - 1, panelY, panelX + PANEL_WIDTH, panelY + panelHeight, COLOR_BORDER_DARK);
        ...
        for (Object[] line : lines) {
            if (line == null) {
                ...
                extractor.text(font, pctStr, textX, curY, COLOR_PROGRESS_TEXT, false);
                curY += LINE_HEIGHT;
                ...
                int barH = 5;
                extractor.fill(barX, curY, barX + barW, curY + barH, 0xFF222233);
                int fillW = (int) (barW * pct);
                if (fillW > 0) {
                    extractor.fill(barX, curY, barX + fillW, curY + barH, fullyDone ? 0xFF44DDAA : 0xFF3366BB);
                }
                extractor.fill(barX, curY, barX + barW, curY + 1, 0xFF445566);
                extractor.fill(barX, curY + barH - 1, barX + barW, curY + barH, 0xFF445566);
                curY += barH + 4;
            } else {
                String text = (String) line[0];
                int color = (int) line[1];
                extractor.text(font, text, textX, curY, color, false);
                curY += LINE_HEIGHT;
            }
        }
    }
